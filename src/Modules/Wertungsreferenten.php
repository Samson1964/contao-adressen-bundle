...
class Wertungsreferenten extends \Module
{
	...
	protected function compile()
	{
		...
		$content = '<table class="ce_table">';
		$content .= '<tr>';
		$content .= '<th>Verband/Bezirk</th>';
		$content .= '<th>Referent</th>';
		$content .= '<th>Kontakt</th>';
		$content .= '</tr>';
			...
			while($objAdressen->next())
			{
				...
				if($bezirke)
				{
					foreach($bezirke as $bezirk)
					{
						...
						$temp = '<tr class="##KLASSE##">';
						$temp .= '<td>'.$bezirksname[$bezirk].'</td>';
						$temp .= '<td>'.$objAdressen->vorname.' '.$objAdressen->nachname.'</td>';
						$temp .= '<td>'.$this->FormatiereKontakt($objAdressen).'</td>';
						$temp .= '</tr>';
	...
	function FormatiereKontakt($data)
	{
		...
		{
			$email = array();
			if($data->email1) $email[] = '{{email::'.$data->email1.'}}';
			if($data->email2) $email[] = '{{email::'.$data->email2.'}}';
			if($data->email3) $email[] = '{{email::'.$data->email3.'}}';
			if($data->email4) $email[] = '{{email::'.$data->email4.'}}';
			if($data->email5) $email[] = '{{email::'.$data->email5.'}}';
			if($data->email6) $email[] = '{{email::'.$data->email6.'}}';
		}

		if($telefon) $content = implode(', ', $telefon);
		if($content) $content .= ', ';
		if($email) $content .= implode(', ', $email);
		return $content;
